...
struct Point
{
    ...
    friend Point operator-(const Point& lhs, const Point& rhs) { Point result(lhs); result -= rhs; return result; }
    friend Point operator*(const Point& lhs, const int mul) { Point result(lhs); result *= mul; return result; }
    friend Point operator*(const int mul, const Point& rhs) { Point result(rhs); result *= mul; return result; }
    ...
    for(auto&& [ch, p] : pos)
    {
        ...
        for(int i = 0; i < k; i++)
        {
            for(int j = 0; j < k; j++)
            {
                ...
                auto p_ji = p[j] - p[i];
                auto an = p[i] + p_ji;
                while(an.x >= 0 && an.x < n && an.y >= 0 && an.y < m)
                {
                    anti.insert(an);
                    an += p_ji;
                }
